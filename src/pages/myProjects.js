...
export const myProjectsTemplate = async () => {
    ...
    ul.classList.add('ul-list-style first-list-item');
    ...
    for (let i = 0 ; i < data.length; i++) {
        ...
        if (bool === true) {
            ...
            if (type === 'folder') {
                ...
                ulSub.classList.add('ul-list-style collapsible-items collapse');
                ulSub.id = `toggle${data[i].id}`;
                
                const liSub = document.createElement('li');
                liSub.classList.add('my-prjects-list-item allow-overflow');

                const table = document.createElement('table');
                table.classList.add('table table-borderless table-striped my-projects-table');
                table.caption = `File info table`;

                const thead = document.createElement('thead');

                const theadTR = document.createElement('tr');
                theadTR.classList.add('table-no-wrap');
                theadTR.innerHTML = `
                    <th>Name</th>
                    <th>File id</th>
                    <th>Created by</th>
    ...
    for (let element of Array.from(elements)) {
        element.addEventListener('click', async () => {
            ...
            if (versions.entries.length !== 0) {
                ...
                document.getElementById('modalFVBody').innerHTML = `
                    <table caption="Modal file info table" class="table table-borderless table-striped">
                        <thead>
