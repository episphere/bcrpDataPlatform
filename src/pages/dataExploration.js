...
const filterItemTemplate = (title, values) => {
    if (values.length > 4) {
        return `
            ...
            <span>
                ${values.map((option) => option.replaceAll(/_|-/g, " ")).slice(0, 4).join(", ")}, and other
            </span>
        `;
    } else {
        return `
            <span class="font-bold">${title}:</span>
            <span>
                ${values.length ? values.map((option) => option.replaceAll(/_|-/g, " ")).join(", ") : "None"}
            </span>
        `;
    }
};
...
export const dataSummaryMissingTemplate = async () => {
    ...
    const initialSelection = variables.length > 4 ? variables.slice(0, 4) : variables;
    ...
    */
    ...
    renderFilter(data, initialSelection, Object.keys(cohorts), variables, race, ethnicity);
    midset(data, initialSelection);
    addEventMissingnessFilterBarToggle();
};

const renderFilter = (data, acceptedVariables, acceptedCohorts, headers, race, ethnicity) => {
    let template = "";
    template += `
...
const renderMidsetFilterData = (data, acceptedVariables, acceptedCohorts, headers, race, ethnicity) => {
    let template = "";
    ...
    transformRace.forEach((anc) => {
        ...
        template += `<option value="${anc[0]}" ${anc[1] === "All" ? "selected" : ""}>${anc[1]}</option>`;
    });
    ...
    template += `
        <div class="form-group" id='midsetCohorts'>
            <li class="filter-list-item">
                <label class="filter-label font-size-13" for="studiesList">
                    Cohort<span class='required-label'>*&nbsp;</span>
                </label>
                <label>
                    <input id="cohortallcheckbox" checked type="checkbox" class="cohort-name select-all" title="all"/>
                    Check All
                </label>
            </li>
    `;
    ...
    for (let cohort of acceptedCohorts) {
        cohortsTemplate += `
            <li class="filter-list-item">
                <input type="checkbox"checked data-variable="${cohort}" id="label${cohort}" class="select-cohort"/>
            ...
            <li class="filter-list-item">
                <input type="checkbox" ${acceptedVariables.indexOf(variable) !== -1 ? "checked" : ""} data-variable="${variable}" id="label${variable}" class="select-variable"/>
                    <label for="label${variable}" class="variable-name" title="${variable}">
                        ${variable.replace("_Data available", "").replace(/_|-/g, " ")}
        `;
        
        if (variable === "Reproductive_History1" || variable === "Reproductive_History2") {
            template += `
                <button class="info-btn variable-definition" aria-label="More info" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-target="#confluenceMainModal"  data-variable='${variable}'>
...
const addEventMidsetFilterForm = (data) => {
    ...
    Array.from(cohorts).forEach((ele) => {
        ele.addEventListener("change", () => {
            const selectedCohorts = document.querySelectorAll(".select-cohort:checked");
...
const midset = (data, acceptedVariables) => {
    ...
    if (data.length > 0) {
        ...
        template += `
            <tr class="midset-header">
                <th class="missing-column">
                    <p>Number of subjects with data based on the selection of variables</p>
        ...
        for (let variable in headerCount) {
            template += `<th class="missing-column cell-equal-width">${variable.replace("_Data available", "").replace(/_|-/g, " ")}</th>`;
        }
