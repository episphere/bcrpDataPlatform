...
import { getSelectedStudies, renderAllCasesCharts, renderAllCharts, updateCounts, getFileContent, getFileContentCases } from "./visualization.js";
    ...
    allEntries.forEach((entry) => {
        ...
        tbody += `
            <tr>
                ...
                <td>
                    ${email !== JSON.parse(localStorage.parms).login && userPermission && updatePermissionsOptions(userPermission, role) && userName === addedBy ? `
                        <select title="Update permission" data-collaborator-id="${id}" data-previous-permission="${role}" data-collaborator-name="${name}" data-collaborator-login="${email}" class="form-control updateCollaboratorRole">
                ...
                <td title="${new Date(addedAt).toLocaleString()}">
                    ${new Date(addedAt).toDateString()}
                </td>
                <td>
                    ${addedBy === userName ? `
                        <button class="removeCollaborator" title="Remove collaborator" data-collaborator-id="${id}" data-email="${email}" data-collaborator-name="${name}" data-folder-name="${folderName}">
...
const addEventSearchCollaborators = (allEntries, userPermission) => {
    ...
    search.addEventListener("input", () => {
        ...
        filteredEntries = allEntries.filter((dt) =>
            dt.name.toLowerCase().replace(/(<b>)|(<\/b>)/g, "").includes(searchValue) ||
            dt.email.toLowerCase().replace(/(<b>)|(<\/b>)/g, "").includes(searchValue)
        );
...
const updatePermissionsOptions = (userPermission, role) => {
    ...
    else if (userPermission === "editor" && role !== "co-owner" && role !== "owner") {
        return `<option ${
        role === "editor" ? `selected` : ``
        } value="editor">editor</option><option ${
        role === "viewer" ? `selected` : ``
            ...
            if (r) {
                ...
                if (response.status === 204) {
                    ...
                    let template = notificationTemplate(
                        ...
                        `Collaborator ${name || email} removed from ${folderName} successfully!`
                    );
                ...
                for (let index = 0; index < emails.length; index++) {
                    ...
                    if (response.status === 200 || response.status === 201) {
                        ...
                        template += notificationTemplate(
                            ...
                            `Could not add ${login} to ${name} as ${role.value}, <span class="errorMsg">${(await response.json()).message}</span>!!`
                        );
                    }
                }
            }
        ...
        document.getElementById("modalFileStatsBody").innerHTML = `
            <div class="row file-stats-row">
                <div class="col" title="File download count">
                    <span class="file-stats-heading">Download count</span>
                    </br>
                ...
                <div class="col" title="File edit count">
                    <span class="file-stats-heading">Edit count</span>
                    </br>
            ...
            <div class="row file-stats-row">
                <div class="col" title="File preview count">
                    <span class="file-stats-heading">Preview count</span>
                    </br>
                ...
                <div class="col" title="File comment count">
                    <span class="file-stats-heading">Comment count</span>
                    </br>
...
export const addEventUpdateSummaryStatsData = () => {
    ...
    btn.addEventListener("click", async () => {
        ...
        template += `
            ...
            <div style="padding-left:40px"><input type="radio" required value="summary" name="summarydataType">
                &nbsp;Summary data
            </div>
            <div style="padding-left:40px"><input value="missingness" required type="radio" name="summarydataType">
                &nbsp;Missingness data
            </div>
        `;
...
const addEventDataTypeRadio = () => {
    ...
    Array.from(radios).forEach((radio) => {
        radio.addEventListener("click", async () => {
            ...
            if (dataType === "summary") {
                summaryFolder = response.entries.filter((dt) =>
                    dt.type === "folder" &&
...
const addEventUpdateSummaryStatsForm = () => {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        for (let id of folderIds) {
            ...
            if (dataType === "summary") {
                file = response.entries.filter((dt) =>
                    dt.type === "file" &&
...
const filterData = (jsonData, headers) => {
    ...
    if (ethnicity !== "all") { finalData = finalData.filter((dt) => dt["ethnicity"] === ethnicity); }
    if (study !== "all") { finalData = finalData.filter((dt) => dt["study"] === study); }
    if (race !== "all") { finalData = finalData.filter((dt) => dt["race"] === race); }
    ...
    let totalSubjects = 0;
    finalData.forEach((value) => { totalSubjects += value.TotalSubjects; });
    
    if (subCases == "all") { renderAllCharts(finalData); }
...
export function switchTabs(show, hide, files) {
    try {
        ...
        else if (!Array.isArray(files)) { return; }
...
export function filterCheckBox(table, data) {
    ...
    if (currentValue.length <= 2 && (previousValue.length > 2 || previousValue.length === 0)) {
        ...
        searchedData = JSON.parse(JSON.stringify(filteredData)).filter((dt) => {
            let found = false;
            if (dt.name.toLowerCase().includes(currentValue)) found = true;
            if (dt.created_by.name.toLowerCase().includes(currentValue)) found = true;
            if (dt.parent.name.toLowerCase().includes(currentValue)) found = true;
            if (found) return dt;
        });
    }
